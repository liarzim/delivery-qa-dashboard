...
### Fixed (post-release)
- **Widget Builder sheet selector** — Sheet field is now always visible. Previously it was only rendered when the sheets API returned 2+ names, so any API failure or single-sheet file silently hid the field. Now: if the API returns sheet names a dropdown shows; otherwise a text input appears with placeholder "Sheet name (leave blank for default)" so users can always type a sheet name manually. Additionally: backend `/api/data/raw/sheets/:source` now returns SheetNames for all three sources (was delivery-only); page fetches sheets for any dataSource; switching sources resets sheet/xField/yField.
